...
action :install do
  ...
  # Install ssl frontend file
  template "/etc/haproxy/haproxy.ssl" do
    source "haproxy.ssl.erb"
    cookbook "lb_haproxy"
    owner "haproxy"
    group "haproxy"
    mode "0400"
...
action :attach do
  ...
  if node[:lb_haproxy][:ssl] == 'true'
    ...
    template ::File.join("/etc/haproxy/ssl/", new_resource.backend_id) do
      source "haproxy_server_ssl.erb"
      owner "haproxy"
      group "haproxy"
      mode "0600"
      backup false
...
action :detach do
  ...
  # remove ssl version
  if node[:lb_haproxy][:ssl] == 'true'
    file ::File.join("/etc/haproxy/ssl/", backend_id) do
      action :delete
      backup false
      notifies :run, resources(:execute => "/etc/haproxy/haproxy-cat.sh")
    end
